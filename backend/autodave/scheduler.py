...
def fire_trigger(store: Store, engine: Engine, a: dict, t: dict) -> bool:
    ...
    True when an execution actually started. The live-check and the start (or
    the skipped record) happen under one lock, so a firing can never race a
    concurrent manual start into a second live execution or a lost skip."""
    ...
    def _tick(self) -> None:
        ...
        for a in autos:
            ...
            if retry and retry[0] <= now:
                ...
                with self.store.lock:
                    old = None if a.get("_live") else self.store._latest_exec(a["id"])
                    # §6/§7: the auto-retry is in place — same record, new
                    # attempt, never a new execution. If the record is no longer
                    # the failed one (the user already executed again), the
                    # retry simply lapses.
                    if old and old["status"] == "failed":
                        try:
                            ...
                            pass
        # Forget baselines/retry state of deleted automations / removed triggers.
        auto_ids = {a["id"] for a in autos}
        self._baseline = {k: v for k, v in self._baseline.items() if k in live_keys}
        self._retry_at = {k: v for k, v in self._retry_at.items() if k in auto_ids}
        self._retried &= auto_ids
